...
public class SchemaEvolutionValidator {
    ...
    private static final Schema NULL_TYPE_SCHEMA = Schema.create(Type.NULL);
    ...
    /* package private */void compareTypes(Schema oldSchema,
        ...
        Type oldType = oldSchema.getType();
        Type newType = newSchema.getType();
    ...
    /* package private */void compareRecordTypes(Schema oldSchema,
                                                 ...
                                                 List<Message> messages,
                                                 String name) {
        if (oldSchema == null || newSchema == null || oldSchema.getType() != Type.RECORD) {
    ...
    /* package private */void compareUnionTypes(Schema oldSchema,
                                                ...
                                                List<Message> messages,
                                                String name) {
        if (oldSchema == null || newSchema == null || oldSchema.getType() != Type.UNION) {
    ...
    /* package private */void compareEnumTypes(Schema oldSchema,
                                               ...
                                               List<Message> messages,
                                               String name) {
        if (oldSchema == null || newSchema == null || oldSchema.getType() != Type.ENUM) {
            throw new IllegalArgumentException("Old schema must be ENUM type. Name=" + name
                    + ". Type=" + oldSchema);
        }

        if (newSchema.getType() != Type.ENUM) {
            messages.add(new Message(Level.ERROR, "Illegal type change from " + oldSchema.getType()
                    + " to " + newSchema.getType() + " for field "
                    + name));
            return;
    ...
    /* package private */void compareArrayTypes(Schema oldSchema,
                                                ...
                                                List<Message> messages,
                                                String name) {
        if (oldSchema == null || newSchema == null || oldSchema.getType() != Type.ARRAY) {
            throw new IllegalArgumentException("Old schema must be ARRAY type. Name=" + name
                    + ". Type=" + oldSchema);
        }

        if (newSchema.getType() != Type.ARRAY) {
            messages.add(new Message(Level.ERROR, "Illegal type change from " + oldSchema.getType()
                    + " to " + newSchema.getType() + " for field "
                    + name));
            return;
    ...
    /* package private */void compareMapTypes(Schema oldSchema,
                                              ...
                                              List<Message> messages,
                                              String name) {
        if (oldSchema == null || newSchema == null || oldSchema.getType() != Type.MAP) {
            throw new IllegalArgumentException("Old schema must be MAP type. Name=" + name
                    + ". Type=" + oldSchema);
        }

        if (newSchema.getType() != Type.MAP) {
            messages.add(new Message(Level.ERROR, "Illegal type change from " + oldSchema.getType()
                    + " to " + newSchema.getType() + " for field "
                    + name));
            return;
    ...
    /* package private */void compareFixedTypes(Schema oldSchema,
                                                ...
                                                List<Message> messages,
                                                String name) {
        if (oldSchema == null || newSchema == null || oldSchema.getType() != Type.FIXED) {
            throw new IllegalArgumentException("Old schema must be FIXED type. Name=" + name
                    + ". Type=" + oldSchema);
        }

        if (newSchema.getType() != Type.FIXED) {
            messages.add(new Message(Level.ERROR, "Illegal type change from " + oldSchema.getType()
                    + " to " + newSchema.getType() + " for field "
                    + name));
            return;
    ...
    /* package private */void comparePrimitiveTypes(Schema oldSchema,
        ...
        if (oldType != newType) {
            if (((oldType == Type.INT && (newType == Type.LONG
                    || newType == Type.FLOAT || newType == Type.DOUBLE))
                    || (oldType == Type.LONG && (newType == Type.FLOAT || newType == Type.DOUBLE)) || (oldType == Type.FLOAT && (newType == Type.DOUBLE)))) {
