...
public class SchemaManager {
    ...
    private static class SchemaElement {
        private AvroEntitySchema entityDef;
    ...
    private static class CacheEntity {
        ...
        public AvroEntitySchema get(SchemaVersion version, long timeout) {
        ...
        public void put(SchemaVersion version, AvroEntitySchema entityDef) {
            SchemaElement se = versions.get(version);
            if (se == null) {
    ...
    private AvroEntitySchema checkCache(SchemaEntity schemaEntity, SchemaVersion version) {
        if (cache != null) {
    ...
    public AvroEntitySchema checkAndSave(@NonNull AvroEntitySchema schema,
        ...
        AvroEntitySchema current = get(schemaEntity, version);
        if (current == null) {
    ...
    public AvroEntitySchema save(@NonNull AvroEntitySchema schema,
    ...
    private SchemaVersion nextVersion(SchemaEntity entity,
        ...
        if (current != null) {
            ...
            List<AvroEntitySchema> schemas = findSchemas(entity);
    ...
    public AvroEntitySchema get(@NonNull SchemaEntity schemaEntity) throws Exception {
        return get(schemaEntity, currentVersion(schemaEntity));
    }

    public AvroEntitySchema get(@NonNull SchemaEntity schemaEntity,
                          @NonNull SchemaVersion version) throws Exception {
        AvroEntitySchema en = checkCache(schemaEntity, version);
    ...
    public AvroEntitySchema get(@NonNull SchemaEntity schemaEntity,
        ...
        if (client.checkExists().forPath(path) != null) {
            ...
            if (data != null && data.length > 0) {
                ...
                AvroEntitySchema en = JSONUtils.read(data, AvroEntitySchema.class);
                en.load();
