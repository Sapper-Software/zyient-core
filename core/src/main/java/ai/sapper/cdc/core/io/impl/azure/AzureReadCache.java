...
public class AzureReadCache implements EvictionCallback<String, AzureReadCache.CacheFile> {
    ...
    public AzureReadCache(@NonNull AzureFileSystem fs) {
        this.fs = fs;
    }

    public AzureReadCache init(@NonNull HierarchicalConfiguration<ImmutableNode> xmlConfig) throws ConfigurationException {
    ...
    public AzurePathInfo get(@NonNull AzurePathInfo path) throws Exception {
        LRUCache<String, CacheFile> cache = get(path.container());
    ...
    public File put(@NonNull AzurePathInfo path, long updated) throws Exception {
        synchronized (cache) {
            LRUCache<String, CacheFile> cache = get(path.container());
            Optional<CacheFile> cf = cache.get(path.path());
            if (cf.isPresent()) {
    ...
    private LRUCache<String, CacheFile> get(String container) {
        synchronized (cache) {
