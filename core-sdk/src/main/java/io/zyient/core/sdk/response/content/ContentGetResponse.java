...
public class ContentGetResponse extends ContentResponse<ContentGetRequest> {
    ...
    public URL parseUrl() throws Exception {
        if (Strings.isNullOrEmpty(response.getDownloadUrl())) {
            throw new Exception(String.format("[request=%s][id=%s] URL is not specified. ",
                    getRequest().getRequestId(), getRequest().getContentId()));
        }
        return new URL(response.getDownloadUrl());
    }
}
