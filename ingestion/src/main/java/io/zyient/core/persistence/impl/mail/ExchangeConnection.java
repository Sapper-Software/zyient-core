...
import io.zyient.core.persistence.impl.settings.mail.ExchangeConnectionSettings;
...
public class ExchangeConnection extends AbstractMailConnection<ExchangeService> {
    ...
    public String getEmailId() {
        Preconditions.checkState(settings instanceof ExchangeConnectionSettings);
        return ((ExchangeConnectionSettings) settings).getEmailId();
    }

    public String getMailServer() {
        Preconditions.checkState(settings instanceof ExchangeConnectionSettings);
        return ((ExchangeConnectionSettings) settings).getMailServer();
    }

    public int getPort() {
        Preconditions.checkState(settings instanceof ExchangeConnectionSettings);
        return ((ExchangeConnectionSettings) settings).getPort();
    }

    public String getUsername() {
        Preconditions.checkState(settings instanceof ExchangeConnectionSettings);
        return ((ExchangeConnectionSettings) settings).getUsername();
    }

    public String getDelegateUser() {
        Preconditions.checkState(settings instanceof ExchangeConnectionSettings);
        return ((ExchangeConnectionSettings) settings).getDelegateUser();
    }
    ...
    public boolean useDelegate() {
        Preconditions.checkState(state.getState() == Connection.EConnectionState.Initialized || state.isConnected());
        ExchangeConnectionSettings settings = (ExchangeConnectionSettings) settings();
    ...
    public Connection connect() throws ConnectionError {
        Preconditions.checkState(state.getState() == Connection.EConnectionState.Initialized);
        try {
