...
public class ExchangeDataStore extends AbstractMailDataStore<ExchangeService, EmailMessage, Folder> {
    ...
    public String getMailUser() {
        if (exchangeConnection != null) {
            return exchangeConnection.getEmailId();
        }
        return null;
    }
        ...
        if (elems != null && elems.size() > 0) {
            for (Element elm : elems) {
                ...
                if (celems == null || celems.size() < 0)
                    continue;
                MailUtils.MimeContent content = saveImageToLocal(elm, count);
                if (content != null) {
    ...
    private MailUtils.MimeContent saveImageToLocal(Element node, int index) throws Exception {
    ...
    @Override
    public AbstractMailMessage<EmailMessage> createMessage(@Nonnull String mailId,
    ...
    private Folder findFolder(WellKnownFolderName root,
                              Folder parent,
                              String[] parts,
                              int index,
                              boolean create)
            ...
            if (parent == null) {
                ...
                if (kf != null) {
                    ...
                    if (exchangeConnection.useDelegate()) {
                        fid = new FolderId(kf, new Mailbox(exchangeConnection.getDelegateUser()));
                        folder = Folder.bind(exchangeConnection.connection(),
                                new FolderId(kf, new Mailbox(exchangeConnection.getDelegateUser())));
                    } else {
                        fid = new FolderId(kf);
                    ...
                    if (exchangeConnection.useDelegate()) {
                        fid = new FolderId(root, new Mailbox(exchangeConnection.getDelegateUser()));
                    } else {
                    ...
                    if (results != null && results.getFolders().size() > 0) {
                        ...
                        if (exchangeConnection.useDelegate()) {
                            FolderId fi = new FolderId(root, new Mailbox(exchangeConnection.getDelegateUser()));
                            folder.save(fi);
                        } else
                            folder.save(root);
                    }
    ...
    private Folder createFolder(Folder parent, String[] parts, int index) throws DataStoreException {
        try {
            ...
            if (root == null) {
                if (exchangeConnection.useDelegate()) {
                    FolderId fi = new FolderId(rootFolder, new Mailbox(exchangeConnection.getDelegateUser()));
    ...
    public Folder createFolder(@Nonnull String name) throws DataStoreException {
        ...
        try {
            ...
            if (parts.length == 1) {
                ...
                if (exchangeConnection.useDelegate()) {
                    parent = Folder.bind(exchangeConnection.connection(), new FolderId(rf,
                            new Mailbox(exchangeConnection.getDelegateUser())));
                } else {
    ...
    private Collection<AbstractMailMessage<EmailMessage>> fetch(int batchSize,
                                                                int offset) throws DataStoreException {
        try {
            if (offset < 0)
                offset = 0;
            if (batchSize <= 0)
                batchSize = settings.getMaxResults();
            ...
            if (findResults.getItems() != null && !findResults.getItems().isEmpty()) {
                ...
                for (Item item : findResults) {
                    if (item instanceof EmailMessage) {
                        ...
                        EmailMessageWrapper ew = new EmailMessageWrapper(exchangeConnection.getEmailId(), em, true);
                        messages.add(ew);
                    }
                }
                return messages;
    ...
    public MessageWrapper readEmailFile(@Nonnull String path) throws DataStoreException {
        ...
        try {
            ...
            return new MessageWrapper(exchangeConnection.getEmailId(), message);
    ...
    public void configure() throws ConfigurationException {
        ...
        try {
            Preconditions.checkArgument(connection() instanceof ExchangeConnection);
            exchangeConnection = (ExchangeConnection) connection();
    ...
    public <E extends IEntity<?>> E findEntity(@Nonnull Object key,
        ...
        try {
            ...
            if (findResults.getItems() != null && !findResults.getItems().isEmpty()) {
                ...
                if (item instanceof EmailMessage) {
                    return (E) new EmailMessageWrapper(exchangeConnection.getEmailId(), (EmailMessage) item, true);
                }
            }
            return null;
        } catch (Exception ex) {
    ...
    public Map<String, Integer> getFolderMessageCounts(String parent) throws DataStoreException {
        try {
            DefaultLogger.info(String.format("Getting folder message count for [mailbox= %s]",
                    this.getMailUser()));
            ...
            if (folder != null) {
                ...
                if (children != null && children.length > 0) {
                    for (Folder child : children) {
                        ...
                        DefaultLogger.info(String.format("Checking folder : [%s]", fname));
                ...
                int mc = folder.getTotalCount();
                if (mc > 0) {
                    DefaultLogger.info(String.format("Folder message count : [%s][%d]", parent, mc));
    ...
    public void cleanUpEmptyFoldersBasedOnTimePeriod(String processingFolderPath, String folderPath, int timePeriod) throws DataStoreException {
        try {
            ...
            if (folder != null) {
                Folder[] children = findChildFolders(folder, FolderTraversal.Shallow);
                if (children != null && children.length > 0) {
                    DefaultLogger.debug(String.format("sub folders found for folder: %s", folder.getDisplayName()));
                    for (Folder child : children) {
                        ...
                        DefaultLogger.debug(String.format("checking completed for folder : [%s]", fname));
                    }
                    //all children are processed now
                    DefaultLogger.debug(String.format("All subfolders checked for [%s]", folderPath));
                    ...
                    if (childrenAfterProcessing == null || childrenAfterProcessing.length == 0) {
                        DefaultLogger.debug(
                                String.format("The folder [%s] does not lead to any emails, hence will be deleted", folderPath));
    ...
    private boolean withinTimePeriod(int timePeriod, String folderPath) {
        ...
        DefaultLogger.debug(String.format("Date information obtained from path %s, [date=%s]", folderPath, dateString));
        ...
        try {
            ...
            DefaultLogger.debug(
                    String.format("[timePeriod limit=%d hours] [detected time difference = %d hours]",
                            timePeriod, timeDiffInHours));
            if (timeDiffInHours <= timePeriod) isWithinTimePeriod = true;
        } catch (ParseException e) {
            DefaultLogger.warn(String.format("Parsing exception: %s", e.getLocalizedMessage()));
        }
    ...
    private void checkAndDelete(Folder folder) throws ServiceLocalException, DataStoreException {
        DefaultLogger.warn(String.format("Cleaning folder [%s]...", folder.getDisplayName()));
        Folder[] children = findChildFolders(folder, FolderTraversal.Shallow);
        if (children != null && children.length > 0) {
            for (Folder f : children) {
                checkAndDelete(f);
        ...
        if (isEmpty(folder)) {
            DefaultLogger.info(String.format("Deleting empty folder. [folder=%s]", folder.getDisplayName()));
            if (!deleteFolder(folder)) {
                DefaultLogger.error(
                        String.format("Failed to delete folder. [folder=%s]", folder.getDisplayName()));
            }
        } else {
            DefaultLogger.info(String.format("Folder is not empty. [folder=%s]", folder.getDisplayName()));
        }
    }
}
